...
async def show_user_channels(query):
    ...
    if not channels:
        await query.edit_message_text("😕 You haven't added this bot to any channel yet. Please Again /start")
        return
...
async def unmute_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # If not restricted, no need to unmute
        if member.status != "restricted":
            return await message.reply_text("ℹ️ User is not muted.")

        # If already allowed to send messages
        if member.can_send_messages:
            return await message.reply_text("ℹ️ User is already unmuted.")
        ...
        logger.error(f"/unmute error: {e}")
        await message.reply_text("❌ Failed to unmute the user.")
