...
async def show_user_groups(query):
    ...
    if not groups:
        await query.edit_message_text("😕 You haven't added this bot to any group yet.")
        return
...
async def show_user_channels(query):
    ...
    if not channels:
        await query.edit_message_text("😕 You haven't added this bot to any channel yet.")
        return
    ...
    kb.append([InlineKeyboardButton("🏠 Menu", callback_data="force_start")])
    await query.edit_message_text("📢 Your Channels:", reply_markup=InlineKeyboardMarkup(kb))
...
async def apply_action(filter_type: str, chat_id: int, user_id: int, message, context):
    ...
    if action == "mute":
        ...
        await message.reply_text(f"🔇 User muted for {format_duration(duration)}.", quote=False)

    elif action == "ban":
        until_date = datetime.utcnow() + duration
        await context.bot.ban_chat_member(chat_id, user_id, until_date=until_date)
        await message.reply_text(f"🚫 User banned for {format_duration(duration)}.", quote=False)
        ...
        await message.reply_text(f"⚠️ Warning {warn_count}/{max_warn}.", quote=False)
        ...
        if warn_count >= max_warn:
            if s.get("post_warn_action", "mute") == "ban":
                ...
                await message.reply_text(f"🚫 Banned after {warn_count} warnings.", quote=False)
            else:
                ...
                await message.reply_text(f"🔇 Muted after {warn_count} warnings.", quote=False)
...
async def unmute_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not message.reply_to_message:
        return await message.reply_text("🔓 Reply to a message to unmute the user.")
    ...
    try:
        ...
        if member.status != "restricted":
            return await message.reply_text("ℹ️ This user is not muted.")
