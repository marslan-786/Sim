...
async def message_filter_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ✅ کسٹم میسجز چیک کریں
        elif settings.get("custom_messages") and actions.get("custom", {}).get("enabled"):
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("add_custom_message_"):
            gid = int(data.rsplit("_", 1)[1])
            user_custom_add[uid] = gid  # یوزر کی شناخت یاد رکھو کہ یہ کس گروپ کے لیے سینڈ کر رہا ہے
...
if __name__ == "__main__":
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, custom_message_input_handler))
