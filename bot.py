...
async def global_channel_settings(query: Update.CallbackQuery, context: ContextTypes.DEFAULT_TYPE):
    user_id = query.from_user.id
...
async def toggle_forward_removal(query: Update.CallbackQuery, context: ContextTypes.DEFAULT_TYPE):
    user_id = query.from_user.id
    current = channel_forward_settings.get(user_id, {}).get("remove_forward_tag", False)
    channel_forward_settings[user_id]["remove_forward_tag"] = not current

    # Refresh menu
    await global_channel_settings(query, context)
